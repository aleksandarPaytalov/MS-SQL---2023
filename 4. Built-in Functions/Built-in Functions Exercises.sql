...
SELECT PeakName, 
	   RiverName, 
	   LOWER (CONCAT(PeakName, SUBSTRING(RiverName, 2, 100))) AS Mix
FROM Peaks AS p
...
SELECT TOP(50) [Name] AS [GameName], 
CAST([Start] AS DATETIME2) AS [StartDate]
FROM Games
WHERE DATEPART(YEAR,[Start]) IN (2011, 2012)
ORDER BY [StartDate], [GameName]

SELECT TOP 50
		   [Name] as GameName,
		   CONVERT(varchar, [Start], 23) AS [StartDate]
FROM Games
...
SELECT TOP(50) 
		  [Name] AS GameName,
		  FORMAT([Start], 'yyyy-MM-dd', 'bg-BG') AS StartDate
FROM Games
WHERE YEAR([Start]) IN (2011,2012)
ORDER BY [StartDate], GameName
